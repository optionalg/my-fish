function fish_greeting --description 'Print the shell greeting'
    ...
    set -l perlbrew_vers ( printf "%s" (perl -E "print(\$ARGV[1] =~ s/.*Name:\s+([\d\.]*)/\$1/r)" (perlbrew info)) )
    set -l perlbrew (printf "%sCurrent perlbrew version in use is %s%s%s." $cn $cc $perlbrew_vers $cn)
